(() => {
  ...
  const closeBtn = document.getElementById("owMenuClose");
  ...
  const openMenu = () => {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        document.body.classList.add("ow-menu-open");
      });
    });
  ...
  const closeMenu = () => {
    document.body.classList.remove("ow-menu-open");
    button.setAttribute("aria-expanded", "false");
    button.setAttribute("aria-label", "メニューを開く");
    document.body.classList.remove("ow-no-scroll");

    window.setTimeout(() => {
      if (!document.body.classList.contains("ow-menu-open")) {
        menu.hidden = true;
        overlay.hidden = true;
      }
    }, 260);
