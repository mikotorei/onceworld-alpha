...
function saveSsCalc() {
  ...
  try {
    localStorage.setItem(SS_CALC_KEY, JSON.stringify({
      ...
      scrollCount:   $("ss-scroll-count")?.value     || "0",
...
function ssTotalStatPoints(lv, tenme, hasCosmoCube, penCount, altarCount, tenshoCount, scrollCount) {
...
function ssUpdateStatPointDisplay() {
  ...
  const pts = ssTotalStatPoints(lv, tenme, ssHasCosmoCube, penCount, altarCount, tenshoCount, scrollCount);
...
// 振り分けポイント計算のイベント登録
["ss-chara-lv", "ss-sp-tenme-count", "ss-pen-count", "ss-altar-count", "ss-tensho-count", "ss-scroll-count"].forEach(id => {
