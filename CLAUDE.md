...
- 仕様が曖昧な箇所は推測で進めず、選択肢を示して確認する
- 実装前に影響範囲と変更内容を報告し、承認を得てから着手する
- 段階を分けられる作業はコミットを分ける
- 既存の計算結果が変わる変更は、変更前後を比較して検証する
- 完了後はコミット・プッシュまで行う
- 作業中に見つけた別の不具合は、勝手に直さず報告する

## 停止条件

「要判断」で止まるのは失敗ではない。次のときは値を勝手に変えずに止めて報告する。

- 指示の前提が実物と食い違っているとき
- 仕様判断が必要で、選び方で表示や計算結果が変わるとき
- 直し方が2つ以上あるとき

止まるときは「選べる道」も書く。影響範囲を並べた案が付いていれば、判断が1往復で済む。

ただし、指示に書かれていないことのすべてが停止条件ではない。

| 種類 | 扱い |
|---|---|
| 選び方で計算結果や表示が変わる | 止めて報告 |
| 既存の実装を流用すれば一意に決まる | 決めて、決めた内容と理由を報告に書く |

「値を勝手に変えて条件を満たす」ことは絶対にしない。

実際に、指示より実物のほうが正しかったケースが繰り返し起きている。
止まったときは、実装ではなく指示のほうが間違っている可能性をまず報告する。

## 報告

チャットに長文を出さない。ファイルに逃がす。

- 報告を `_reports/YYYY-MM-DD_HHmm_作業名.md` に保存する
- チャット上には「保存しました: ファイル名」と、要判断があればその見出しだけ返す
- `_reports/` は `.gitignore` に入れてあるためコミットされない

報告に含める内容:
- やったこと（簡潔に）
- 指示から変更した点とその理由
- 自分で決められなかった判断（あれば「要判断」として明記）
- 検証結果

## 出力の作法

- 値を変更するときは、変更前の値を出力してから変更する（`set 上限 1100 -> 100`）
- 比較するときは複数箇所を同じ形式で並べ、違う行に `DIFF` を付ける
- 「設定した」と「意図どおりになった」は別。変更後に実値を読んで出力する
- 判定の基準は、指示に書かれた定数ではなく実測値から取る
...
1. 消す対象の中身を出力する
2. 外からそれを参照している箇所を全ファイル走査で確認する
3. 参照が1件でもあれば削除せず「要判断」として報告する
4. 参照が0件なら削除
5. 削除後に件数を出力する
...
- 2回実行して壊れないこと
- 「同じ名前がある」だけでスキップしない。中身まで確認する
- 算出式に自分自身が含まれていないか確認する

## 検証の標準手順

計算式を変える場合:
1. 変更前の実装を git show で復元
2. 同じ入力で新旧の結果を比較（境界値・通常値・異常系）
3. 不一致0件を確認してから報告

HTML構造を変える場合:
1. hugo でビルド
2. 出力HTMLを変更前と比較（可能ならバイト単位）
3. id / class の集合が変わっていないか確認
...
```
通常強化の上限 = 100 + 禁域のロック所持数
  ロック0個 → 100 / 1000個 → 1100 / 2000個（パンドラ所持）→ 2100
G強化の上限   = 300（固定）
G強化の解禁条件 = 通常強化 +1100（固定。通常強化の上限とは連動しない）
...
```
粉の使用上限       = 100 + ドラゴン印の手ごね機の所持数
ペットの最大レベル = 1200 + ハデスの兜の所持数（2200で頭打ち）
```

- 定義は `game-data.js` の `LIMITS.petPowder` / `LIMITS.petLevel`。
  値は `getLimit(limitId, materialCounts)` で取る
- 素材の所持数は `pet-sim.js` の `materialCapOf(id)` / `ownedCount(input, id)` に通して
  パンドラ込みの上限に収める。**素材ごとに上限取得を書かないこと**
  （兜だけパンドラ対応・手ごね機は未対応、という取り違えが実際に起きた）
- ペットステシミュの粉欄は `applyPowderMax()` が入力欄の `max`・各行のMAXボタンの文言・
  「全部MAX」の入力値をまとめて上限に追従させる
- 必要経験値計算機のペットLv上限は `exp-calc.js` の `petLvMax()`。
  ハデスの兜の入力欄は `petHelmet`（`MATERIALS` に slot を宣言済み）
- ハデスの兜は2200で頭打ちのため、パンドラを持っていても最大レベルは変わらない
...
| ゴッドオブデビルアイ | クリティカル倍率 +0.003/個（定数1.5あり） | 最大 5.5倍 | 最大 8.5倍 |
| 死刑囚の身代わり晩餐 | 主人公への被ダメージ -0.09%/個 | 最大 -90% | 最大 -180%（0で底打ち） |
| ドラゴン印の手ごね機 | ペットの粉使用上限 +1/個 | 最大 1,100 | 最大 2,100 |
| ハデスの兜 | ペットの最大レベル +1/個 | 最大 Lv2200 | 最大 Lv2200（頭打ち） |
| 属性キノコ（青・赤・緑・光・闇） | 属性が一致するペットの最も高いステータス +1/個 | 最大 +1,000 | 最大 +2,000 |
| 魔晶立方体 | 解析書補正後のINT +1%/個 | 最大11倍 | 最大21倍 |
| 解析書 | 魔法ダメージのINTに +1/冊（解析書の解析書の倍率が乗る） | 最大 +101,000 | 最大 +402,000 |
| 解析書の解析書 | 解析書の効果 +10%/個 | 最大 ×101 | 最大 ×201 |
...
✅ `calc-logic.js` の各計算は `materialCap()` 経由でパンドラの状態を加味した上限を使い、
`guide-floor-calc.js` の天空像も連動する。
ハードコードされていた 2000 / 10000 / 80000 / 1000 / 11.0 はすべて置き換えた。
`pet-sim.js` も `materialCapOf()` に集約済み（粉・ハデスの兜・キノコ）。

⚠️ `exp-calc.js` には `Math.min(1000, …)` が残っている。
ペガサスのメダル（`medalCount`）／ジパングの酒（`zipangCount`）／
ルミナスキノコ（`luminousCount`）／古のティラピス像（`heroTilapia` `petTilapia`）の4種で、
パンドラを持っていても2000個を入力できない。
同じページでもハデスの兜（`petHelmet`）だけは `materialCap()` 経由になっている。
...
- 通常強化 … 装備の +1100 など。G強化とは別系統（上限は「装備の強化上限」を参照）
- G強化 … 通常強化1100到達後に解禁される強化
- 指定ワープ … 天空回廊の最高到達-10万Fワープ
- 麒麟ワープ … 天空回廊の100万F直行ワープ
- SG … スカイガーディアン（100の倍数フロアに出現）
- 2個置き … 天空像～冒険者～を2個外して片側撃破し、回収して進む操作
...
- ソラノアエキスの効果量（モンスター生成時の弱体率）
- 勇者装備のセットボーナス
- 共有の焚火・時空歪曲装置の所持数依存
- 煌命輪 / スティールウィング / 三炎燭 の max_level
- 1万・10万・100万Fのボスモンスター（1000万Fは ID230 ゲーム外生命体X）
...
- ボスモンスターのステータス
- 装備DBでの表示欄
- ボスフロア判定を game-data.js へ集約し、天空回廊計算表でも活用

## 保留中の判断

- `exp-calc.js` の `Math.min(1000, …)` 4種（ペガサスのメダル／ジパングの酒／
  ルミナスキノコ／古のティラピス像）はパンドラ未対応のまま。
  重要度が低いため当面は現状維持と判断（2026-09-21）。
  直すとパンドラ所持時の入力上限が2倍になり計算結果が変わるので、
  指示があるまで触らないこと
